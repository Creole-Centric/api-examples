"""
Webhook Routes for CreoleCentric TTS API
...
@webhook_bp.route('/webhooks/tts', methods=['POST'])
def webhook_receiver():
    ...
    It validates the signature, updates the job status, and can trigger
    additional actions (notifications, etc.)
    ...
    try:
        ...
        # Validate signature
        if not validate_webhook_signature(request.get_data(), signature, timestamp):
