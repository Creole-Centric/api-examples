# CreoleCentric Streaming TTS — Python Example
...
The streaming endpoint applies these constraints:
...
flag between PCM chunks and exits cleanly with a `cancelled` SSE
event.

## SSE event shapes

The endpoint emits these event types (all wrapped in `data: {...}\n\n`):
...
`done` events also carry `word_timings` (forced-alignment data for
subtitles) when alignment is available.
